function checkCashRegister(price, cash, cid) {
  ...
  for (var j = 0; j < drawerCash.length; j++){
    if (change/denom[j] >= 1){
      var bill;
      // if the change is divisible by a denomination keep subtracting the change from cash in drawer until it is no longer divisible or there are no more cash in the drawer.
